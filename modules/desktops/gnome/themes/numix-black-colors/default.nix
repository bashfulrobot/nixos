...
let
  cfg = config.desktops.gnome.themes.numix-black-colors;
  numix-black-colors-gtk-theme = pkgs.callPackage ./build { };

in {
  options = {
    desktops.gnome.themes.numix-black-colors.enable = lib.mkOption {
      type = lib.types.bool;
      default = false;
      description = "Enable numix-black-colors gtk Theme.";
    };
  };
  ...
  config = lib.mkIf cfg.enable {
    home-manager.users."${user-settings.user.username}" = {
      ...
      home.packages = with pkgs; [ numix-black-colors-gtk-theme ];
