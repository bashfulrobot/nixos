{ user-settings, osConfig, pkgs, config, lib, inputs, ... }:
let

  cfg = config.desktops.hyprland.waybar;
  hostname = builtins.getEnv "HOSTNAME"; # Get hostname from environment variable
...
in {
  options = {
    desktops.hyprland.waybar.enable = lib.mkOption {
      type = lib.types.bool;
      default = false;
      description = "Enable waybar in Hyprland Desktop";
    };
  };
    ...
    home-manager.users."${user-settings.user.username}" = {
      ...
      programs.waybar = {
        ...
        settings = [{
          ...
          modules-left =
            [ "sway/workspaces" "sway/mode" "hyprland/workspaces" ];
          modules-right = [
            # "wlr/taskbar"
            "pulseaudio"
            "bluetooth"
