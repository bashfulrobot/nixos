...
let
  ...
  cfg = config.desktops.hyprland.theme.gruvbox;
  gruvboxPlus = import ./build/gruvbox-plus.nix { inherit pkgs; };

in {

  options = {
    desktops.hyprland.theme.gruvbox.enable = lib.mkOption {
      type = lib.types.bool;
      default = false;
      description = "Enable gruvbox theme in Hyprland Desktop";
    };
  };
  config = lib.mkIf cfg.enable {
    ...
    home-manager.users."${user-settings.user.username}" = {
      ...
      qt = {
        ...
        style = {
          name = "adwaita-dark";
          # TODO: is there one that will work for adw-gtk3?
