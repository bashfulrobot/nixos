...
in {
    ...
    options.programs.firefox.webapps = mkOption {
        ...
        attrsOf (submodule {
          options = {
            ...
            name = mkOption {
              ...
              default = null;
              description =
                "Specific name of the application. Defaults to the capitalized attribute name.";
            };
      ...
      description =
        "Websites to create special site-specific Firefox instances for.";
    };
    ...
    config = {
      ...
      xdg.desktopEntries = mapAttrs (name: cfg: {
        inherit (cfg)
          genericName comment categories icon mimeType prefersNonDefaultGPU;

        name = if cfg.name == null then
          (toUpper (substring 0 1 name))
        ...
        exec = concatStringsSep " " ([
          ...
          "--no-remote"
        ] ++ cfg.extraArgs ++ [ "${cfg.url}" ]);

        settings = {
          X-MultipleArgs =
            "false"; # Consider enabling, don't know what this does
